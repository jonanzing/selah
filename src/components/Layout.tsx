    ...
    <div className="min-h-screen selection:bg-gold/30 selection:text-gold-light transition-colors duration-500">
      ...
      >
        ...
        >
          <div className="w-8 h-8 transition-transform group-hover:scale-110 pointer-events-none">
            <img 
              src={get('asset_favicon', '/favicon.svg')} 
              alt="Logo" 
              className="w-full h-full object-contain"
              referrerPolicy="no-referrer"
            />
          </div>
          <span className="pointer-events-none">{get('site_name', 'Selah')}<span className="text-green">.</span></span>
        </div>
        <ul className="hidden md:flex items-center gap-10 list-none">
          ...
          <li className="flex items-center gap-4">
            ...
            <a 
              href={get('btn_waitlist_url', '/#waitlist')} 
            ...
            >
              {get('btn_waitlist_text', 'Join waitlist')}
            </a>
          </li>
        </ul>
        <div className="md:hidden flex items-center gap-3">
          ...
          <a href={get('btn_waitlist_url', '/#waitlist')} className="bg-green text-black px-4 py-2 rounded-full text-xs font-medium">{get('btn_waitlist_text_short', 'Join')}</a>
        </div>
      </nav>
      ...
      <footer className="bg-surface border-t border-border px-6 lg:px-16 py-12 flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="flex items-center gap-2 font-serif text-2xl font-medium tracking-tight">
          ...
          <span>{get('site_name', 'Selah')}<span className="text-green">.</span></span>
        </div>
        ...
        <div className="text-xs text-text-dim">© 2026 {get('site_name', 'Selah')}. All rights reserved.</div>
      </footer>
    </div>
  );
};
